...
import { Camera, Gamepad2, Palette, ShieldCheck, Trash2, UserRound } from "lucide-react";
...
function passwordErrorsFrom(details: ErrorDetail[]): PasswordErrors {
  return details.reduce<PasswordErrors>((errors, detail) => {
    ...
    if (["currentPassword", "newPassword", "confirmPassword"].includes(field) && !errors[field])
...
export function ProfilePage() {
  ...
  async function handlePasswordSubmit(event: FormEvent) {
    ...
    if (newPassword === currentPassword)
      clientErrors.newPassword = "A nova senha deve ser diferente da senha atual.";
  ...
  async function handlePreferencesSubmit(event: FormEvent) {
    ...
    try {
      const updatedUser = await api.profile.updatePreferences(preferredPlatforms);
      synchronizeUser(updatedUser);
      setPreferredPlatforms(updatedUser.preferredPlatforms);
      toast.success("Preferências atualizadas.");
    } catch (error) {
      toast.error(
        error instanceof ApiError ? error.message : "Não foi possível atualizar as preferências.",
      );
    } finally {
  ...
  async function handleAvatarChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      toast.error(error instanceof ApiError ? error.message : "Não foi possível enviar a imagem.");
    } finally {
      setUploading(false);
    }
  }
  ...
  async function handleAvatarRemoval() {
    ...
    try {
      ...
      toast.error(error instanceof ApiError ? error.message : "Não foi possível remover a imagem.");
    } finally {
      setRemoving(false);
    }
  }
    ...
    <div className="mx-auto max-w-4xl space-y-8">
      <div>
        ...
        {(user.preferredPlatforms ?? []).length > 0 && (
          <div className="mt-4 flex flex-wrap gap-2" aria-label="Plataformas preferidas">
      ...
      <section className="relative overflow-hidden rounded-3xl bg-brand px-6 py-7 text-brand-foreground sm:px-8">
        ...
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center">
          ...
          <div>
            ...
            <h2 className="mt-2 text-3xl font-bold tracking-[-0.04em]">{user.name}</h2>
            <p className="mt-1 text-sm text-brand-foreground/55">{user.email}</p>
          </div>
        </div>
      </section>
      ...
      <div className="grid items-start gap-4 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="space-y-4">
          <Card className="border-brand/12 shadow-[0_14px_40px_-34px_#17313a]">
            ...
            <CardContent>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                >
                  <Camera />
                  {uploading ? "Enviando..." : user.avatarUrl ? "Trocar foto" : "Enviar foto"}
                </Button>
                {user.avatarUrl && (
                  <Button
                    type="button"
          ...
          <Card className="border-brand/12 shadow-[0_14px_40px_-34px_#17313a]">
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground">
                Tema {theme === "dark" ? "escuro" : "claro"} em uso neste dispositivo.
              </p>
      ...
      <Card className="border-brand/12 shadow-[0_14px_40px_-34px_#17313a]">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Escolha onde você costuma jogar. Essas preferências aparecem como badges no seu perfil.
          </p>
        </CardHeader>
        <CardContent>
          <form onSubmit={handlePreferencesSubmit} className="space-y-5">
            ...
            <Button
              ...
              disabled={
                preferencesSaving ||
                (preferredPlatforms.length === (user.preferredPlatforms ?? []).length &&
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-5">
            <div className="space-y-2">
              ...
              <PasswordInput
                ...
                onChange={(event) =>
                  updatePasswordField("currentPassword", event.target.value, setCurrentPassword)
                }
            ...
            <div className="space-y-2">
              ...
              <PasswordInput
                ...
                value={newPassword}
                onChange={(event) =>
                  updatePasswordField("newPassword", event.target.value, setNewPassword)
                }
              ...
              <p id="new-password-help" className="text-xs text-muted-foreground">
                Use de 8 a 72 caracteres, com letra maiúscula, minúscula e número.
              </p>
            ...
            <div className="space-y-2">
              ...
              <PasswordInput
                ...
                onChange={(event) =>
                  updatePasswordField("confirmPassword", event.target.value, setConfirmPassword)
                }
                autoComplete="new-password"
