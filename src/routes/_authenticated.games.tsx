  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold">Catálogo de jogos</h1>
          <p className="text-sm text-muted-foreground">Cadastre jogos para usar na fila dos grupos.</p>
        </div>
        <CreateGameDialog />
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input placeholder="Buscar jogos..." value={search} className="pl-9"
      ...
      {query.error && <p className="text-sm text-destructive">{(query.error as ApiError).message}</p>}

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {query.data?.games.map((g) => <GameCard key={g.id} game={g} />)}
      </div>
      ...
      {query.data && query.data.meta.totalPages > 1 && (
        <div className="flex items-center justify-center gap-3">
          ...
          <Button variant="outline" size="sm" disabled={page >= query.data.meta.totalPages}
    ...
    <Card className="overflow-hidden">
      ...
      <CardContent className="p-4">
        ...
        <div className="mt-2 flex flex-wrap gap-1">
          {game.platforms.map((p) => <Badge key={p} variant="secondary">{p}</Badge>)}
          {game.maxPlayers && <Badge variant="outline">até {game.maxPlayers} jogadores</Badge>}
        </div>
        {game.description && (
      ...
      {editing && <EditGameDialog game={game} open={editing} onOpenChange={setEditing} />}
    </Card>
  );
}
...
function GameForm({
  ...
  onSubmit: (data: {
    title: string; platforms: Platform[]; maxPlayers: number | null;
  ...
  const [maxPlayers, setMaxPlayers] = useState<string>(initial?.maxPlayers?.toString() ?? "");
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-2">
          ...
          <Input type="number" min={1} value={maxPlayers}
      ...
      <div className="space-y-2">
        ...
        <Textarea maxLength={1000} value={description ?? ""}
...
function CreateGameDialog() {
  ...
  const mutation = useMutation({
    mutationFn: (data: Parameters<typeof api.games.create>[0]) => api.games.create(data),
    onSuccess: () => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button><Plus className="mr-2 h-4 w-4" /> Novo jogo</Button>
      </DialogTrigger>
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Cadastrar jogo</DialogTitle>
          <DialogDescription>O título é normalizado e deve ser único.</DialogDescription>
        </DialogHeader>
        <GameForm submitting={mutation.isPending} submitLabel="Cadastrar"
...
}: { game: Game; open: boolean; onOpenChange: (v: boolean) => void }) {
  const qc = useQueryClient();
  const mutation = useMutation({
    mutationFn: (data: Parameters<typeof api.games.update>[1]) => api.games.update(game.id, data),
    onSuccess: () => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        ...
        <GameForm initial={game} submitting={mutation.isPending} submitLabel="Salvar"
          onSubmit={(data) => mutation.mutate(data)} />
      </DialogContent>
    </Dialog>
  );
